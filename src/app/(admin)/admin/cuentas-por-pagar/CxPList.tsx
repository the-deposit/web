...
import { registerCxPPayment, createManualCxP } from "./actions";
...
const SOURCE_TYPE_LABELS: Record<string, string> = {
  ...
  consignacion_recibida: "Consignación recibida",
};

interface CxPListProps {
  accounts: Account[];
}

export function CxPList({ accounts: initialAccounts }: CxPListProps) {
  ...
  const [manualForm, setManualForm] = useState({ creditor_name: "", total_amount: "", due_date: "", notes: "" });
  ...
  const handleConfirmPayment = () => {
    ...
    startTransition(async () => {
      const res = await registerCxPPayment({
        account_id: payingId,
  ...
  const handleCreateManual = () => {
    if (!manualForm.creditor_name || !manualForm.total_amount) {
      setFormError("Acreedor y monto son requeridos.");
      return;
    }
    startTransition(async () => {
      const res = await createManualCxP({
        creditor_name: manualForm.creditor_name,
      ...
      if (res.success) {
        ...
        setManualForm({ creditor_name: "", total_amount: "", due_date: "", notes: "" });
  ...
  return (
    <div className="space-y-4">
      <div className="flex items-center justify-between">
        <h1 className="font-display text-2xl text-primary uppercase">Por Pagar</h1>
      ...
      <div className="bg-secondary border border-border rounded p-4">
        <p className="text-xs font-body text-gray-mid uppercase tracking-wide mb-1">Total pendiente por pagar</p>
      ...
      {filtered.length === 0 ? (
        <EmptyState icon={DollarSign} title="Sin cuentas" description="No hay cuentas por pagar con ese filtro." />
      ) : (
                  ...
                  <div className="flex flex-wrap items-center gap-2 mb-1">
                    ...
                    {a.source_type && (
                      <Badge variant="outline" className="text-[10px]">
                        {SOURCE_TYPE_LABELS[a.source_type] ?? a.source_type}
                      </Badge>
                    )}
                  </div>
        ...
        <div className="fixed inset-0 z-50 flex items-end md:items-center justify-center">
          ...
          <div className="relative z-10 w-full md:max-w-md bg-secondary rounded-t-2xl md:rounded-lg md:mx-4">
            <div className="sticky top-0 bg-secondary border-b border-border p-4 flex items-center justify-between">
              <h3 className="font-display text-lg text-primary">Nueva cuenta por pagar</h3>
              <button onClick={() => setShowManualForm(false)} className="text-gray-mid hover:text-primary text-xl">×</button>
            </div>
            <div className="p-4 space-y-3">
              <Input label="Acreedor / Proveedor" required value={manualForm.creditor_name} onChange={(e) => setManualForm((p) => ({ ...p, creditor_name: e.target.value }))} placeholder="A quién le debemos" />
