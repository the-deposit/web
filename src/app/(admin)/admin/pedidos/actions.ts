...
export async function updateOrderStatus(formData: unknown) {
  ...
  const { orderId, status, notes_internal, estimated_delivery } = parsed.data;
  ...
  // If delivered or picked up, convert to sale
...
async function convertOrderToSale(supabase: SupabaseClient, orderId: string, sellerId: string) {
  const { data: order } = await supabase
    .from("orders")
    .select(`
      id, customer_id, subtotal, shipping_cost, total, address_id,
